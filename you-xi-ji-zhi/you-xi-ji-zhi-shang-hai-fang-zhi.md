...
2. 瞬间型伤害防止效果不会追踪要吸收的伤害量，而是会试图防止部分或全部一次伤害。在这种情况下，即使被防止的伤害量为零，只要仍然存在一次防止伤害的尝试，该防止的实例也仍会被消耗。如果伤害超过了某个防止实例所设定的数量，剩余的伤害将正常造成。

防止效果会按其效果所指定的方式持续存在。

在面对不可防止的伤害时，瞬间型防止效果仍将试图防止该伤害。不可防止的伤害不会减少护盾型防止效果（见上文）。
