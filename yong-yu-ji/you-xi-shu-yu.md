...
4. 一件非礼装武器，如果其上的耐久指示物数量因战斗或其他方式而变为 0，则会因状态效应而被消灭。
5. 一个单位，会因状态效应而在其生命值小于或等于 0，或其上标记的伤害（临时的或通过伤害指示物）等于或大于其生命值时被消灭。
...
1. 死亡是一个用来指代场上单位将要进入墓场的术语。场上的单位在被一个有来源的效果（例如，与一个物件相关联的效果）直接消灭，或因状态效应，例如生命值小于或等于 0 而消灭时，便会死亡。
