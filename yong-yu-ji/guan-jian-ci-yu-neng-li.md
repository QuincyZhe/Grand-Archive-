...
4. 由横扫攻击所发起的战斗步骤中，防御伙伴在攻击宣告期间被指定。任何于此之后在防御玩家操控下进入场上的单位，都不会被指定为防御者，也不会作为防御者参与战斗。
...
3. 如果一张带有指令能力的攻击卡牌将被置入一个意图，作为状态行动，会检查攻击者的物件类型是否符合该能力指定的任何子类型。如果指令能力指定的子类型未被满足，且意图中的指令对于该攻击者而言不是一次合规的攻击，则该攻击卡牌会因状态效应而被置入墓场。
...
2. 赋能被赋予给操控着那个赋予赋能效果的卡牌或效果的玩家。它作为一个持续性效果被应用，并在被赋能的玩家启动其下一张法术卡牌时立刻被消耗并应用。
...
1. 受阻是一种静止式能力，其意思是“此物件以待命状态进入场上。”。
2. 受阻是冗余的。
...
1. 不朽是一种静止式能力，其意思是“此物件不能从场上被送往墓场。”。
...
2. 带有链接能力的物件，在从效果堆叠经结算进入场上时，会与一个被指定为目标的物件进入链接状态，该目标通常位于“链接”一词之前。
   1. 如果一个带有链接能力的物件将要从效果堆叠以外的区域进入场上，则在它进入场上时，只需选择要进入链接状态的物件即可。此过程不指定目标。
   2. 如果带有链接能力的卡牌在启动或具现时没有合规目标，则会失效。如果带有链接能力的物件从效果堆叠以外的区域进入场上，且没有合规的链接对象可供选择，则它将被牺牲（见第6和第7条）。
...
4. 当带有链接能力的物件或处于链接状态的物件任意一方离开场上时，链接能力便会断开。
5. 如果因处于链接状态的物件以任何方式离开场上而导致链接被破坏，则带有链接能力的物件会作为一个状态行动被牺牲（玩家必须在任何时机被传递以及任何后续玩家行动被做出之前，将其牺牲）。
...
3. 放逐时从场上触发，并将被放逐的那个物件，而非被置入放逐区的那张卡牌本身，视作其能力的来源。
...
3. 死亡时从场上触发，并将死亡的那个物件，而非被置入墓场的那张卡牌本身，视作其能力的来源。

### [On Enter](#user-content-fn-27)[^27]

1. 参战时是物件的一种触发式能力条件，其意思是“当此项进入场上时。”。
2. 对于英雄而言，参战时会将进入传承并成为最上方英雄卡牌的那张牌，视作如同该卡牌进入了场上一样（它不是一个新的物件）。
...
1. 击杀时是一种在被攻击的单位因一次攻击而被击杀时的触发式能力条件。
   1. 单位因生命值小于或等于0，会作为状态效应而被击杀。
   2. 可攻占的领地因耐久小于或等于0，会作为状态效应而被消灭，但由于它们不被视为单位，因此不会触发击杀时。
2. 在单位上，击杀时的意思是“当一个单位因此来源造成的战斗伤害而被击杀时”。
3. 在意图和武器上，击杀时的意思是“当一个单位在一次使用此项的攻击中，因受到战斗伤害而被击杀时。”

### [On Leave](#user-content-fn-31)[^31]

1. 离场时是一种触发式能力条件，其意思是“当此项离开场上时”。
2. 离场时从场上触发，并将离开游戏的那个物件，而非被移至另一个区域的那张卡牌本身，视作其能力的来源。
...
1. 可再生是礼装的一种静止式能力，其意思是“如果此牌将要从场上或意图中被放逐，则改为将其置入其拥有者的素材库。”
