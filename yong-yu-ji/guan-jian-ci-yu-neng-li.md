...
{% hint style="warning" %}
卡牌印刷上的能力提示文本可能与综合规则中的规则条目存在差异。提示文本旨在简化规则表述，提升游戏体验的清晰度与便利性。为精简广泛能力的不必要规则描述，卡牌印刷时可能对提示文本进行调整。具体规则请参阅本章节。
{% endhint %}
...
### [Command Will](#user-content-fn-10)[^10] N

1. 号令意志是棋兵单位的静止式能力，其意思是“只要此单位使用号令卡牌攻击，其力量+N”。
2. 当该单位的意图区存在号令攻击卡牌时，此能力条件即满足。

### [Divine Relic](#user-content-fn-11)[^11]
...
### [Element Bonus](#user-content-fn-13)[^13]
...
2. 瞬逝代价以“—”分隔。与此代价或能力相关的任何修正内容均列于此代价之后。
3. 由于瞬逝属于替换启动机制，不会获得特殊权限或时机窗口（不同于星界引召），其启动必须遵循卡牌的默认速度。
4. 通过此能力使用的伙伴进入战场时即获得瞬逝状态。
5. 瞬逝状态是一种特性，规定瞬逝物件离开战场时将会被放逐。
6. 瞬逝状态（作为物件特性）不会适用于该物件所代表的卡牌。若卡牌需移动区域，该卡牌不会保留瞬逝状态，且后续由该卡牌代表的物件（除非另有说明）将不具有瞬逝状态。

{% hint style="success" %}
若效果会放逐一张瞬逝状态的卡牌，因其目标区域未发生改变，该卡牌仍能追踪该物件。例如，任何形式的临时放逐后返回战场的机制（如压制）均允许该卡牌回到战场。若通过此类方式返回，该卡牌将被视为新物件，不会“记住”其曾具有瞬逝状态；新物件不具有瞬逝状态。
{% endhint %}

### [Floating Memory](#user-content-fn-15)[^15]

1. 遥忆是一种静止式能力，其意思是“在支付回忆代价时，你可以从你的墓场放逐此牌来支付该代价中的1点。”。
2. 在支付回忆代价时，以遥忆支付的部分必须在确定作为支付回忆代价的一部分而需随机放逐的卡牌数量之前宣告。
3. 遥忆是冗余的。

### [Fast Activation](#user-content-fn-16)[^16]
...
### [Inherited effect](#user-content-fn-21)[^21]
...
### [Lineage Release](#user-content-fn-25)[^25]
...
### [On Attack](#user-content-fn-28)[^28]
...
### [On Banish](#user-content-fn-30)[^30]
...
### [On Charge N](#user-content-fn-31)[^31]
...
### [On Death](#user-content-fn-32)[^32]
...
### [On Leave](#user-content-fn-37)[^37]
...
### [True Sight](#user-content-fn-51)[^51]
