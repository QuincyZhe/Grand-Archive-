...
2. 英雄奖励在所有区域中起作用。
3. 若具有英雄奖励能力的牌列出名称与玩家所操控的英雄的名称有差异的英雄，使得职业奖励的条件不被符合，则与该奖励相关的关键词和效果作为规则文本的一部分在每个区域中被忽略。
...
1. 虚化是一种静止式能力，其意思是“你可以支付此卡牌的虚化代价，将其从墓场启动。”。
   1. 虚化代价以“—”分隔。与此代价或能力相关的任何修正内容均列于此代价之后。
   2. 由于虚化属于替换启动机制，不会获得特殊权限或时机窗口（不同于星界引召），其启动必须遵循卡牌的默认速度。
2. 通过此能力使用的伙伴进入战场时即获得虚化状态。
   1. 虚化状态是一种特性，规定虚化物件离开战场时将会被放逐。
   2. 虚化状态（作为物件特性）不会适用于该物件所代表的卡牌。若卡牌需移动区域，该卡牌不会保留虚化状态，且后续由该卡牌代表的物件（除非另有说明）将不具有虚化状态。

{% hint style="success" %}
若效果会放逐一张虚化状态的卡牌，因其目标区域未发生改变，该卡牌仍能追踪该物件。例如，任何形式的临时放逐后返回战场的机制（如压制）均允许该卡牌回到战场。若通过此类方式返回，该卡牌将被视为新物件，不会“记住”其曾具有虚化状态；新物件不具有虚化状态。
{% endhint %}
